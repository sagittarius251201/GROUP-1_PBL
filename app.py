...
pdf_insights = {}; pdf_figs = {}

# 1. Visualization
if page == "Visualization":
    st.header("📊 Visualization")
    c1,c2,c3 = st.columns(3)
    avg = df.SpendPerServing.mean(); trial=(df.TryNewBrand=='Yes').mean()*100; subs=(df.SubscribePlan=='Yes').sum()
    c1.metric("Avg Spend", f"AED {avg:.2f}")
    c2.metric("Trial Rate", f"{trial:.1f}%")
    c3.metric("Subscribers", subs)
    ...
    if chart == "Scatter":
        ...
        insight = f"Correlation between {x} and {y}."
    ...
    pdf_insights["Visualization"] = insight; pdf_figs["Visualization"] = fig
...
# 2. Classification
    ...
    mets = {"Accuracy":accuracy_score(ye,preds),"Precision":precision_score(ye,preds),"Recall":recall_score(ye,preds),"F1-score":f1_score(ye,preds)}
    st.table(pd.DataFrame.from_dict(mets, orient='index', columns=['Value']))
    cm = confusion_matrix(ye,preds)
    fig_cm = go.Figure(go.Heatmap(z=cm, x=["Pred No","Pred Yes"], y=["Act No","Act Yes"], text=cm, texttemplate="%{text}", colorscale="Blues"))
    ...
    pdf_insights["Classification"] = insight; pdf_figs["Confusion Matrix"] = fig_cm; pdf_figs["ROC Curve"] = fig_roc
...
# 3. Clustering
    ...
    km = KMeans(n_clusters=k_val, random_state=42).fit(df[feats])
    ...
    pdf_insights["Clustering"] = insight; pdf_figs["Elbow Method"] = fig_elbow; pdf_figs["Cluster Centers"] = fig_centers
...
# 4. Association
    ...
    pdf_insights["Association"] = insight; pdf_figs["Top Rules"] = fig
...
# 5. Anomaly
    ...
    fig = px.scatter(df, x="MonthlyDisposableIncome", y="SpendPerServing", color=df["Anomaly"].map({1:"Normal",-1:"Anomaly"}), template=tpl)
    st.plotly_chart(fig, use_container_width=True)
    insight = f"Outliers detected: {(df.Anomaly==-1).sum()}."
    pdf_insights["Anomaly"] = insight; pdf_figs["Anomaly"] = fig
...
# 6. Regression
    ...
    st.plotly_chart(fig, use_container_width=True)
    insight = "Regression model comparison."
    pdf_insights["Regression"] = insight; pdf_figs["Regression"] = fig
...
# 7. Forecasting
    ...
    if len(ts) > 30:
        ...
        pdf_insights["Forecasting"] = insight; pdf_figs["Forecast"] = fig
    else:
...
# 8. Cohort
    ...
    if mode == "Time":
        ...
        pdf_insights["Cohort Time"] = insight; pdf_figs["Cohort Time"] = fig
    else:
        feats = ["Age","MonthlyDisposableIncome","SpendPerServing","HealthConsciousness"]
        k = st.slider("Clusters for Cohort", 2, 8, 4)
        labels = KMeans(n_clusters=k, random_state=42).fit_predict(df[feats])
        df["CohortCluster"] = labels
        ...
        st.plotly_chart(fig, use_container_width=True)
        insight = "Subscription by cluster."
        pdf_insights["Cohort Cluster"] = insight; pdf_figs["Cohort Cluster"] = fig

# 9. Geography
elif page == "Geography":
    st.header("🗺️ Geography")
    stats = df.groupby("City")["SpendPerServing"].mean().reset_index()
    fig = px.bar(stats, x="City", y="SpendPerServing", color="SpendPerServing", template=tpl)
    st.plotly_chart(fig, use_container_width=True)
    insight = "Average spend by city."
    pdf_insights["Geography"] = insight; pdf_figs["Geography"] = fig
...
# 10. Sentiment
    ...
    pdf_insights["Sentiment"] = insight; pdf_figs["Word Cloud"] = fig; pdf_figs["Sentiment Pie"] = fig2
...
# 11. LTV & Churn
    ...
    st.plotly_chart(fig, use_container_width=True)
    insight = "LTV distribution and churn metrics."
    pdf_insights["LTV & Churn"] = insight; pdf_figs["LTV"] = fig
...
# 12. Price Elasticity
    ...
    insight = f"At AED {price}, {buyers} buyers, revenue AED {rev}."
...
# 13. Chat
    ...
    try:
        ...
        if "msgs" not in st.session_state:
            st.session_state.msgs = [{"role":"system","content":"You are a data analyst."}]
        ...
        st.warning("Install openai in requirements.txt to enable chat.")

# 14. Glossary
elif page == "Glossary":
    st.header("📚 Glossary")
    glossary = {
        "SpendPerServing":"AED per serving",
        "HealthConsciousness":"Health importance (1–10)",
        "TryNewBrand":"Will try new brand",
