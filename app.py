...
def generate_pdf(insights, figs):
    ...
    # Figures
    for i, (caption, fig) in enumerate(figs.items(), 1):
        img_bytes = fig.to_image(format="png", width=600, height=400)
        img = io.BytesIO(img_bytes)
        story.append(Paragraph(f"<b>Figure {i}: {caption}</b>", styles["Heading3"]))
...
if st.sidebar.button("📄 Download Analysis (PDF)"):
    ...
    insights = {
        ...
        "Trial Rate": f"{(df.TryNewBrand=='Yes').mean()*100:.1f}% of respondents would try a new brand.",
        "Subscription Rate": f"{(df.SubscribePlan=='Yes').mean()*100:.1f}% are subscribed.",
        # add more key takeaways per tab...
    }
    # 2) Regenerate figures
    figs = {
        "Spend vs Income": px.scatter(df, x="MonthlyDisposableIncome", y="SpendPerServing",
                                      template=tpl, title="Spend vs Income"),
        "Spend Distribution": px.histogram(df, x="SpendPerServing", nbins=30,
                                           template=tpl, title="Spend Distribution"),
        # ... include one figure per tab ...
    }
    pdf_bytes = generate_pdf(insights, figs)
    st.download_button("Save PDF report", pdf_bytes,
                       "health_drink_analysis.pdf", "application/pdf")
...
if page == "Visualization":
    ...
    chart = st.selectbox("Chart Type", ["Scatter","Histogram","Box","Bar"], key="viz_chart")
    ...
    if chart == "Scatter":
        ...
        st.markdown(f"**Insight:** Correlation between **{x}** and **{y}** suggests targeting.")
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.markdown(f"**Insight:** **{val}** varies across **{cat}**.")
        ...
        if metric == "Count":
            ...
            st.markdown(f"**Insight:** **{top_seg}** spends highest on average.")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    algo = st.selectbox("Algorithm", ["KNN","Decision Tree","Random Forest","GBRT"], key="clf_algo")
    if algo == "KNN":
    ...
    if hasattr(model, 'feature_importances_'):
        ...
        fig = px.bar(imp, orientation='h', title="Feature Importances",
                     template=tpl, color=imp, color_continuous_scale="Viridis")
        st.plotly_chart(fig, use_container_width=True)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    k = st.slider("Clusters (K)", 2, 8, 4, key="clust_k")
    ...
    a = st.selectbox("Cluster A", prof.index, key="cmp_a")
    b = st.selectbox("Cluster B", prof.index, key="cmp_b")
    st.table((prof.loc[a] - prof.loc[b]).to_frame("Difference").T)
    st.download_button("📥 Download Clusters CSV", df.to_csv(index=False).encode(), "clusters.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    s = st.slider("Min Support", 0.01, 0.2, 0.05, key="assoc_s")
    cth = st.slider("Min Confidence", 0.1, 0.7, 0.3, key="assoc_c")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    if len(ts) > 30:
        ...
        nxt = fc.yhat[-7:].mean()
        st.markdown(f"**Next week avg spend:** AED {nxt:.2f}")
    else:
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    mode = st.radio("Cohort by:", ["Time","Cluster"], key="cohort_mode")
    if mode == "Time":
        ...
        feats = ["Age","MonthlyDisposableIncome","SpendPerServing","HealthConsciousness"]
        k = st.slider("Clusters for Cohort", 2, 8, 4, key="cohort_k")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    fig = px.bar(stats, x="City", y="SpendPerServing", color="SpendPerServing",
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    wc = WordCloud(width=800, height=400,
                   background_color="#333" if is_dark else "#fff").generate(text)
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.plotly_chart(px.histogram(prob, nbins=20, title="Churn Probability", template=tpl),
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    price = st.slider("Price per Serving (AED)", 5, 30, 12, key="price_sim")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    st.header("💬 DataGPT Chatbot")
    try:
        import openai
    except ImportError:
        st.warning("Install `openai` in requirements.txt to enable ChatGPT.")
    else:
