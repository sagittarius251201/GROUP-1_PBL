...
.tooltip {
    border-bottom: 1px dotted var(--tooltip-fg);
    cursor: help;
}
.light { --card-bg: #fff; --tooltip-fg: #555; }
.dark  { --card-bg: #2e2e2e; --tooltip-fg: #ddd; }
</style>
...
def load_data():
    ...
    if p.exists():
        return pd.read_csv(p, parse_dates=["SurveyDate"])
...
if page == "Visualization":
    ...
    c1.markdown(f"<div class='card'><h3>Avg Spend</h3><h2>AED {avg_spend:.2f}</h2></div>", unsafe_allow_html=True)
    c2.markdown(f"<div class='card'><h3>Trial Rate</h3><h2>{trial_rate:.1f}%</h2></div>", unsafe_allow_html=True)
    c3.markdown(f"<div class='card'><h3>Subscribers</h3><h2>{subs}</h2></div>", unsafe_allow_html=True)
    ...
    if chart == "Scatter":
        ...
        st.markdown(f"**Insight:** Positive correlation between **{x}** and **{y}**.")
        ...
        st.plotly_chart(fig, use_container_width=True)
        st.markdown(f"**Insight:** Variation of **{val}** across **{cat}**.")
    else:
        ...
        if metric == "Count":
            ...
            st.markdown(f"**Insight:** Highest spend: **{top}**.")
...
# ─── 2. Classification ────────────────────────────────────────────────────────
    ...
    model.fit(Xt_s, yt)
    preds = model.predict(Xe_s); probs = model.predict_proba(Xe_s)[:,1]
    mets = {
...
# ─── 8. Cohort Analysis ───────────────────────────────────────────────────────
    ...
    if mode=="Time":
        ...
        feats=["Age","MonthlyDisposableIncome","SpendPerServing","HealthConsciousness"]
        k=st.slider("Clusters for Cohort",2,8,4,key="cohk")
...
# ─── 10. Sentiment ────────────────────────────────────────────────────────────
    ...
    wc=WordCloud(width=800,height=400, background_color=("#2e2e2e" if is_dark else "#fff")).generate(text)
...
# ─── 13. ChatGPT Q&A ─────────────────────────────────────────────────────────
    ...
    try:
        ...
        st.warning("Add `openai` to requirements.txt to enable ChatGPT.")
    else:
...
# ─── 14. Glossary ─────────────────────────────────────────────────────────────
    ...
    glossary={
        ...
        "HealthConsciousness":"Importance of health (1–10)",
